...
{
    *x =  x1 + ( checkLg(lg-longitude1) * (x2-x1) / checkLg(longitude2 - longitude1) );
...
{
    *lg =  checkLg(longitude1 + ( (x - x1) * checkLg(longitude2 - longitude1) / (x2-x1) ));
...
QString MapLin::getInfo()
{
    ...
    info = tr("Linear Projection, SCALE 1:%1,\nLat: %2 to %3, Lon: %4 to %5")
           .arg(scale).arg(latitude1*rad2deg).arg(latitude2*rad2deg)
           .arg(longitude1*rad2deg).arg(longitude2*rad2deg);
    return info;
}
...
*/
    QTextOStream os(&param); os.precision(10); os << projection << " " << name << " " << scale << " " << mapSizeX
			 << " " << mapSizeY << " " << longitude1*rad2deg << " " << latitude1*rad2deg << " " << 
	x1 << " " << y1 << " " << longitude2*rad2deg << " " << latitude2*rad2deg << " " << 
	x2 << " " << y2;
...
MapUTM::MapUTM(QString *mapInfo, bool utmCoord):MapBase(mapInfo)
{
    ...
    if(utmCoord)
    {
        ...
        *mapIStream >> utmZone >> utmNorthing1 >> utmEasting1 >> x1 >> y1
        ...
        UTMtoLL(utmNorthing1, utmEasting1, utmZone, latitude1, longitude1, stdLong );
        UTMtoLL(utmNorthing2, utmEasting2, utmZone, latitude2, longitude2, stdLong );
        projection = "UTM";
    }
    else
    {
                     ...
                     double& Lat, double& Long, double& stdLong )
{
    ...
    M = y / k0;
    mu = M/(a*(1-eccSquared/4-3*eccSquared*eccSquared/64-5*eccSquared*eccSquared
               *eccSquared/256));
    ...
    Lat = phi1Rad - (N1*tan(phi1Rad)/R1)*(D*D/2-(5+3*T1+10*C1-4*C1*C1-9*eccPrimeSquared)
...
QString MapUTM::getInfo()
{
    QString info;
    if(universal) {
        info = tr("Universal Transverse Mercator Projection, SCALE 1:%1,\nLat: %2 to %3, Lon: %4 to %5\n"
                  "std.Longitude: %6")
               .arg(scale).arg(latitude1*rad2deg).arg(latitude2*rad2deg)
               .arg(longitude1*rad2deg).arg(longitude2*rad2deg)
               .arg(stdLong*rad2deg);
    } else {
        info = tr("Transverse Mercator Projection, SCALE 1:%1,\nLat: %2 to %3, Lon: %4 to %5\n"
                  "std.Longitude: %6")
               .arg(scale).arg(latitude1*rad2deg).arg(latitude2*rad2deg)
               .arg(longitude1*rad2deg).arg(longitude2*rad2deg)
               .arg(stdLong*rad2deg);
...
/*        param = tr("%1 %2 %3 %4 %5 %6 %7 %8 %9")
   ...
   */
	 QTextOStream os(&param); os.precision(10); os << projection << " " << name << " " << scale << " " << 
mapSizeX << " " << mapSizeY << " " << utmZone << " " << utmNorthing1 << " " << 
utmEasting1 << " " << x1 << " " << y1 << " " << utmNorthing2 << " " << utmEasting2 
<< " " << x2 << " " << y2;
        return param;
    }
    else
...
/*        param = tr("%1 %2 %3 %4 %5 %6 %7 %8 %9")
   ...
   */
	 QTextOStream os(&param); os.precision(10); os << projection << " " << name << " " << scale << " " << 
	    mapSizeX << " " << mapSizeY << " " << longitude1*rad2deg << " " << latitude1*rad2deg 
			     << " " << x1 << " " << y1 << " " << longitude2*rad2deg << " " << latitude2*rad2deg 
			     << " " << x2 << " " << y2 << " " << stdLong*rad2deg;
        return param;
    }
}
...
{
    QString info;
    info = tr("Mercator Projection, SCALE 1:%1,\nLat: %2 to %3, Lon: %4 to %5")
           .arg(scale).arg(latitude1*rad2deg).arg(latitude2*rad2deg)
           .arg(longitude1*rad2deg).arg(longitude2*rad2deg);
    return info;
}
...
MapLambert::MapLambert(QString *mapInfo):MapBase(mapInfo)
{
    ...
    *mapIStream >> longitude1 >> latitude1 >> x1 >> y1
    ...
    // precalc
    n = log( cos(std1Lat) * (1/cos(std2Lat)) ) / log( tan(PI_1_4+std2Lat/2) *
            (-tan(PI_3_4+std1Lat/2)) );
...
*/
	...
	mapSizeX << " " << mapSizeY << " " << longitude1*rad2deg << " " << latitude1*rad2deg 
			 << " " << x1 << " " << y1 << " " << longitude2*rad2deg << " " << latitude2*rad2deg 
			 << " " << x2 << " " << y2 << " " << std1Lat*rad2deg << " " << std2Lat*rad2deg << " " << 
	refLong*rad2deg;
  return param;
}
...
bool MapFritz::calcxy(double *x, double *y, double lg, double lt)
{
    ...
    if ( pixelfact == 0 ) return (FALSE);   /* Added by A. Karhov */
...
bool MapFritz::calcltlg(double *lt, double *lg, double x, double y)
{
    ...
    *lt = zero_lat - y / (Ra[(int) (100 + tempLt)] * deg2rad);
    *lg = zero_long - x / ((Ra[(int) (100 + *lt)] * deg2rad) * cos(deg2rad * *lt));
    dif = *lt * (1 - (cos((deg2rad * fabs(*lg - zero_long)))));
    *lt = *lt - dif / 1.5;
    *lg = zero_long - x / ((Ra[(int) (100 + *lt)] * M_PI / 180.0) * cos(deg2rad * *lt));
    *lt *= deg2rad;
    *lg *= deg2rad;
    return TRUE;
}
...
{
    QString info;
    info = tr("Fritz Projection, SCALE 1:%1,\nCenter Lat: %2, Center Lon: %3")
