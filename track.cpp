...
StartCB->clear();
	...
	if (*curr->name==gpsData->startup_name) StartCB->setCurrentItem(i);
	curr=(Places *)curr->next;
	i++;
    } while ( curr != NULL );
}
...
/* Added by A. Karhov */
	...
	refresh();
    connect( mapLatLonCB, SIGNAL( activated(int) ), this, SLOT( placeSelected(int) ) );
    ...
    tBox = new QHBox(mainBox); tBox->setMargin(2);
    ...
    wBox = new QHBox(mainBox); wBox->setMargin(2);
    wCB = new QCheckBox(wBox); wBox->setMargin(2);
    ...
    rBox = new QHBox(mainBox); rBox->setMargin(2);
    ...
    dBox = new QHBox(mainBox); dBox->setMargin(2);
    ...
    thickList << "1" << "2" << "3" << "4" << "5";
    lBox = new QHBox(mainBox);  lBox->setMargin(2);
...
#if 0
    // CF GPS period
    cBox = new QHBox(mainBox); cBox->setMargin(2);
...
void Track::updateFileList() {
    ...
    if (logdir->exists(gpsData->trackPathStr)) {
        ...
        while ( (fi = it.current())) {
            // add file to list
	    if ( fi->fileName() != "places.txt" &&  fi->fileName() != "places.txt~" ) {
...
void Track::update() {
	...
	if (application->ManualPosit) return; // Added by A. Karhov
...
void Track::drawTrack(QPainter *painter,MapBase *actmap,
    ...
    if (rDo) {
		...
		if ( ! actmap ) return;	/* Added by A. Karhov to fix crash */
                actmap->calcxy(&xwp, &ywp, lg, lt);
                xtp = (int) xwp - x1;
                ytp = (int) ywp - y1;
                if (xtp >= 0 && ytp >= 0 && (int) xwp < mx && (int) ywp <= my) {
                    if (first) {	// first point in read track begins the track
                        painter->moveTo(xtp,ytp); first = false; gpsData->updt_freq = dLE->text().toInt();
    WRITE_CONFIG;
                    } else {			// next are added
