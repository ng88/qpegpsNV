...
GpsStatus::GpsStatus(Qpegps * appl, QWidget * parent, const char *name,
                     WFlags fl):QVBox(parent, name, fl)
{
    application = appl;
    //gpsData = gData;
    ...
    connect(gpsdArgumentsB, SIGNAL(pressed()), SLOT(setGpsdDefaultArg()));
    connect(gpsdArguments, SIGNAL(returnPressed()), SLOT(gpsdArgLEChanged()));

    connect(gpsdHostPortB, SIGNAL(pressed()), SLOT(setGpsdDefaultHostPort()));
    ...
    connect(this, SIGNAL(gpsdArgChanged()), gpsd, SLOT(restartGpsd()));

    connect(d_pStatus, SIGNAL(clicked()), this, SLOT(setSysTime()));
...
void GpsStatus::setGpsdDefaultHostPort()
{
    ...
    gpsd->socket->connectToHost(gpsData->host,
                                gpsData->port);
...
void GpsStatus::updateQuick()
{
    ...
    if (needTextUpdate)
    {
        ...
        if (requestTimeAdj && gpsData->status)
        {
            ...
            QDate tsDate(slst[2].toUInt(), slst[0].toUInt(),
                         slst[1].toUInt());
            QTime tsTime(slst[3].toUInt(), slst[4].toUInt(),
                         slst[5].toUInt());
            ...
            if (tsDT.isValid())
            {
                ...
                qWarning(tr("y=%1,m=%2,d=%3,h=%4,min=%5,s=%6").
                         arg(slst[2].toUInt()).arg(slst[0].toUInt()).
                         arg(slst[1].toUInt()).arg(slst[3].toUInt()).
                         arg(slst[4].toUInt()).arg(slst[5].toUInt()));
    ...
    if (needColorUpdate)
    {
        ...
        d_pStatus->
            setBackgroundColor((gpsData->status ? Qt::green : Qt::red));
    ...
    if (needUpdate)
    {
        // connected ?
        d_pGpsdStatus->
            setBackgroundColor((gpsData->d_connected ? Qt::green : Qt::red));
    ...
    if (gpsData->status)
    {
        ...
        d_pLongitude->setText(Position::longToString(gpsData->longitudeGps));    /* Added by A/ Karhov */
        //if (!d_pLatitude->hasMarkedText())
        d_pLatitude->setText(Position::latToString(gpsData->latitudeGps));       /* Added by A/ Karhov */
    }
    ...
    if (needAliveUpdate)
    {
        // GPS is alive?
        d_pReceiverStatus->
            setBackgroundColor((gpsData->d_aliveGPS ? Qt::green : Qt::red));
...
void GpsStatus::update()
{
    ...
    for (int i = 0; i < 12; ++i)
    {
        ...
        if (!(d_pSatSNR->d_pSatInfo[i] == satInfo) || d_fullUpdate)
        {
            ...
            if (d_fullUpdate)
                d_pSatSNR->d_pSatInfo[i].setUpdated(true);

            needUpdate = true;
        }
    }
    if (needUpdate)
        d_pSatSNR->repaint(false);
...
SatSNR::SatSNR(GpsData * gpsdata, QWidget * parent, const char *name,
               WFlags f):QFrame(parent, name, f), gpsData(gpsdata)
{
}
...
void SatSNR::drawContents(QPainter * painter)
{
    ...
    for (int i = 0; i < 12; ++i)
    {
        ...
        int strength = satInfo.signalNoiseRatio();   //scaled 50pixel == 100%

        int clip = x + WIDTH + xSPC - 1;
        if (clip > max)
            clip = max;

        if (satInfo.satName().isEmpty())
        {
            ...
            for (int p = threshold; p <= Y_ADJ(50); p += Y_ADJ(10))
                painter->drawLine(x, y - Y_ADJ(12) - p, clip,
                                  y - Y_ADJ(12) - p);
...
SatStat::SatStat(GpsData * gpsdata, QWidget * parent, const char *name,
                 WFlags f):QFrame(parent, name, f), gpsData(gpsdata)
{
...
void SatStat::drawSamples()
{
    ...
    for (int index = 0; index < d_numSamples; ++index)
    {
        ...
        if (numsatinfix > numsatinview)
        {
            ...
            painter.drawLine(X_ADJ(18 + index), Y_ADJ(yL1), X_ADJ(18 + index),
                             Y_ADJ(yU1));
        }
        else
        {
            ...
            if (h1 < yL1)
            {
                ...
                painter.drawLine(X_ADJ(18 + index), Y_ADJ(yL1),
                                 X_ADJ(18 + index), Y_ADJ(h1));
            }
            if (h1 > h)
            {
                ...
                if (h1 == yL1)
                    h1 = yL1 + 1;
                painter.drawLine(X_ADJ(18 + index), Y_ADJ(h1 - 1),
                                 X_ADJ(18 + index), Y_ADJ(h));
            }
            if (h > yU1)
            {
                painter.setPen(eraseP);
                painter.drawLine(X_ADJ(18 + index), Y_ADJ(h - 1),
                                 X_ADJ(18 + index), Y_ADJ(yU1));
            }
        }
    }
    ...
    for (int index = 0; index < d_numSamples; ++index)
    {
        ...
        painter.setPen(drawP);
        painter.drawLine(X_ADJ(18 + index), Y_ADJ(yL2), X_ADJ(18 + index),
                         Y_ADJ(h));
        if (h > yU2)
        {
            painter.setPen(eraseP);
            painter.drawLine(X_ADJ(18 + index), Y_ADJ(h - 1),
                             X_ADJ(18 + index), Y_ADJ(yU2));
        }
    }
    ...
    double fAltitude = 0;
    if ((d_maxAltitude - d_minAltitude) > 0)
    ...
    {
        double value =
            (d_pAltitudeSamples[index] - d_minAltitude) * fAltitude;
        int h = yL3 - (int) rint(value);
        painter.setPen(drawP);
        painter.drawLine(X_ADJ(18 + index), Y_ADJ(yL3), X_ADJ(18 + index),
                         Y_ADJ(h));
        if (h > yU3)
        {
            painter.setPen(eraseP);
            painter.drawLine(X_ADJ(18 + index), Y_ADJ(h - 1),
                             X_ADJ(18 + index), Y_ADJ(yU3));
        }
    }
...
void SatStat::shiftSamples()
{
    ...
    double speed = gpsData->speed.toDouble();
    if (speed > d_maxSpeed)
        d_maxSpeed = speed;
    if (speed < d_minSpeed)
        d_minSpeed = speed;
    ...
    double alt = gpsData->altitude.toDouble();
    if (alt > d_maxAltitude)
        d_maxAltitude = alt;
    if (alt < d_minAltitude)
        d_minAltitude = alt;
    ...
    if (!gpsData->d_connected)
        p.second = 99;          // no connection to gpsd? --> force error display
    if (!gpsData->d_aliveGPS)
        p.second = 99;          // no connection to GPS? --> force error display
...
void SatStat::drawContents(QPainter * painter)
{
    ...
    painter->drawRect(xoff, yR1, X_ADJ(d_numSamples + 2),
                      Y_ADJ(d_pixSpeed + 3));


    painter->drawText(0, yR2 + yoff, tr("A:"));
    painter->drawRect(xoff, yR2, X_ADJ(d_numSamples + 2),
                      Y_ADJ(d_pixAltitude + 3));

    painter->drawText(0, yR3 + yoff, tr("#:"));
    painter->drawRect(xoff, yR3, X_ADJ(d_numSamples + 2),
                      Y_ADJ(d_pixSatellites + 3));
    ...
    painter->drawEllipse(compassX - compassOuter / 2,
                         compassY - compassOuter / 2, compassOuter,
                         compassOuter);
    painter->drawEllipse(compassX - compassInner / 2,
                         compassY - compassInner / 2, compassInner,
                         compassInner);
    ...
    {
        tw = (int) ((double) (sin( MathEx::deg2rad(gpsData->heading.toDouble())) * (double) 14.0));
        th = (int) ((double) (cos( MathEx::deg2rad(gpsData->heading.toDouble())) * (double) 14.0));
        sw = -tw / 7;
        sh = -th / 7;
    ...
    {
        tw = (int) ((double) (sin(MathEx::deg2rad(gpsData->bearing.toDouble())) * (double) 28.0));
        th = (int) ((double) (cos(MathEx::deg2rad(gpsData->bearing.toDouble())) * (double) 28.0));
        sw = (int) ((double) (sin(MathEx::deg2rad(gpsData->bearing.toDouble())) * (double) 18.0));
        sh = (int) ((double) (cos(MathEx::deg2rad(gpsData->bearing.toDouble())) * (double) 18.0));
